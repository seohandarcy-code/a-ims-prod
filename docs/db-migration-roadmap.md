...
### `SSO_ALLOW_LOCAL_LOGIN` — client_id 발급 전 개발 부트스트랩 (완료, 2026-09-23)

실제 브로커 연동을 진행하다 보니, 브로커가 `client_id`를 아직 발급하지 않은
단계에서도(사내 정식 SSO 연동 서버에서 나중에 발급 예정) `AUTH_MODE=sso`의
나머지 부분(로그인 게이트, `allowed_users` 접근 제어, "접근 권한 관리" 화면)은
계속 개발/검증할 수 있어야 한다는 필요가 생겼다.

처음엔 "가짜 SSO 로그인" 전용 엔드포인트(비밀번호 없이 `sso_id`만 입력하면
세션을 내주는 방식)를 검토했으나, **이미 있고 검증된 로컬 비밀번호 로그인을
그대로 재사용**하는 쪽으로 바꿨다 — 새 인증 우회 경로를 만드는 것보다 실제
비밀번호 검증을 거치는 기존 경로를 여는 게 더 단순하고 더 안전하다.

- `SSO_ALLOW_LOCAL_LOGIN=true`면 `AUTH_MODE=sso`에서도 `/login`·`/change-password`가
  같이 열린다(`backend/app/api/auth.py`의 `_require_local_login_allowed()`).
  로그인 게이트 화면(`frontend/src/App.vue`)에 실제 "회사 계정으로 로그인"
  버튼과 함께 작은 관리자 비밀번호 폼이 같이 뜬다.
- 흐름: 관리자가 비밀번호로 먼저 로그인 → "접근 권한 관리"에서 SSO 계정들을
  등록 → `client_id`가 발급되면 실제 SSO 버튼으로 로그인해 그 등록이 실제로
  role을 제대로 부여하는지 검증.
- 이 경로로 발급된 세션은 기존 local 로그인과 동일하게 항상 `role="admin"`이고,
  `userName`/`userTeam`이 없다(실제 SSO 콜백만 이 값들을 채운다) — 프론트가 이
  차이로 "실제 SSO로 들어왔는지"를 구분해 접속자 배지/비밀번호 변경 탭 노출을
  결정한다(새 플래그 추가 없이 기존 신호 재사용).
- 기본값 `false`. **공유 서버에서 켤 거면 `ADMIN_BOOTSTRAP_PASSWORD`를 기본값
